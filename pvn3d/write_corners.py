...
pcd = pcl.load('./datasets/'+args.dataset+'/'+dataset_folder[args.dataset]+'/models/obj_'+str(args.cls_id)+'.ply')
pts = np.array(pcd)
...
np.savetxt('./PVN3D/pvn3d/datasets/'+args.dataset+'/'+kps_folder[args.dataset]+'/'+str(args.cls_id)+'/corners.txt', corners)	#corners
pcd = pcl.save(cloud_corners,'./PVN3D/pvn3d/datasets/'+args.dataset+'/'+kps_folder[args.dataset]+'/'+str(args.cls_id)+'/corners.pcd')  ##PCD file with corners added to the actual cloud
