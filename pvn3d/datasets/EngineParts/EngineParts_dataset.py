...
import time
config = Config(dataset_name='EngineParts')
bs_utils = Basic_Utils(config)
DEBUG = False


class EngineParts_Dataset():
    ...
    def __init__(self, dataset_name):
        ...
        self.cls_lst = bs_utils.read_lines(config.EngineParts_cls_lst_p)
        ...
        if dataset_name == 'train':
            self.add_noise = True
            self.path = 'datasets/EngineParts/EngineParts_dataset/train.txt'
            ...
            if os.path.exists(config.preprocessed_testset_pth) and config.use_preprocess:
                ...
                self.path = 'datasets/EngineParts/EngineParts_dataset/test.txt'
                self.all_lst = bs_utils.read_lines(self.path)
        print("{}_dataset_size: ".format(dataset_name), len(self.all_lst))
        self.root = config.EngineParts_root
        self.sym_cls_ids = config.EngineParts_sym_cls_ids
    ...
    def get_item(self, item_name):
        ...
        try:
            ...
            '''with Image.open(os.path.join("./datasets/openDR/openDR_dataset/depth/{}.png".format(item_name) )) as di:
                ...
                dpt = dpt/65535
                dpt = dpt*3.0'''
            with Image.open(os.path.join("./datasets/EngineParts/EngineParts_dataset/mask/{}.png".format(item_name))) as li:
                labels = np.array(li)

                #cls_id_lst = np.unique(labels[labels.nonzero()])
            meta = scio.loadmat(os.path.join('./datasets/EngineParts/EngineParts_dataset/meta/', item_name+'-meta.mat'))
            #if item_name[:8] != 'data_syn' and int(item_name[5:9]) >= 60:
            #    K = config.intrinsic_matrix['openDR']
            #else:
            K = config.intrinsic_matrix['EngineParts']
            ...
            for i, cls_id in enumerate(range(1,1)):
                r = meta['poses'][:, :, i][:, 0:3]
                ...
                ctr = bs_utils.get_ctr(i, ds_type='EngineParts').copy()[:, None]
                ...
                kps = bs_utils.get_kps(
                    i, kp_type=kp_type, ds_type='EngineParts'
                ).copy()
...
def main():
    ...
    ds['test'] = EngineParts_Dataset('test')
    idx = dict(
        train=0,
        val=0,
        test=0
