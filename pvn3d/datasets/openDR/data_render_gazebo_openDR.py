...
for dist in np.arange(0.65,1.05, 0.125):
    ...
    for phi in range(35, 90, 10):
            ...
            for theta in range(0, 360, 15):
                ...
                try:
                     ...
                     resp.append( client('piston_and_conrod', 'world'))
                     resp.append( client('cran_field_round_peg', 'world'))
                     resp.append( client('cran_field_square_peg', 'world'))
                     resp.append( client('cran_field_pendulum', 'world'))
                     resp.append( client('cran_field_pendulum_head', 'world'))
                     resp.append( client('cran_field_separator', 'world'))
                     resp.append( client('cran_field_shaft', 'world'))
                     resp.append( client('cran_field_front_face_plate', 'world'))
                     resp.append( client('Valve_Tappet', 'world'))
                     resp.append( client('M11_50mm_Shoulder_Bolt', 'world'))
                ...
                obj_cam_T = np.zeros((  4, 4, 10 )) # Transformation Mats for 10 object classes
                ...
                for i in np.argsort(-obj_cam_T[2,3,:]):
                    ...
                    this_mask[this_mask.nonzero()] = 1.05*np.ceil(255*(i+1)/10)
