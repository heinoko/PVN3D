...
class openDR_Dataset():
    ...
    def __init__(self, dataset_name):
        ...
        self.cls_lst = bs_utils.read_lines(config.openDR_cls_lst_p)
        ...
        if dataset_name == 'train':
            self.add_noise = True
            self.path = 'datasets/openDR/openDR_dataset/train.txt'
            ...
            if os.path.exists(config.preprocessed_testset_pth) and config.use_preprocess:
                ...
                self.path = 'datasets/openDR/openDR_dataset/test.txt'
                self.all_lst = bs_utils.read_lines(self.path)
        print("{}_dataset_size: ".format(dataset_name), len(self.all_lst))
        self.root = config.openDR_root
        self.sym_cls_ids = config.openDR_sym_cls_ids
    ...
    def get_item(self, item_name):
        ...
        try:
            ...
            '''with Image.open(os.path.join("./datasets/openDR/openDR_dataset/depth/{}.png".format(item_name) )) as di:
                ...
                dpt = dpt/65535
                dpt = dpt*3.0'''
            with Image.open(os.path.join("./datasets/openDR/openDR_dataset/mask/{}.png".format(item_name))) as li:
                labels = np.array(li)
                #cls_id_lst = np.unique(labels[labels.nonzero()])
            meta = scio.loadmat(os.path.join('./datasets/openDR/openDR_dataset/meta/', item_name+'-meta.mat'))
            ...
            for i, val in enumerate([ 26,  53,  79, 107, 133, 160, 186, 214, 240, 11]):
            ...
            for i, cls_id in enumerate(range(1,11)):
                r = meta['poses'][:, :, i][:, 0:3]
                ...
                ctr = bs_utils.get_ctr(i, ds_type='openDR').copy()[:, None]
                ...
                kps = bs_utils.get_kps(
                    i, kp_type=kp_type, ds_type='openDR'
                ).copy()
